...
This can cause table modification methods to be up to `~10×` slower depending on how complex the plum is. **This speed tradeoff should not be noticable in practice** (e.g. it takes `.05 sec` to run `10,000` table modification calls on a nested plum `4` levels deep with over `50` event listeners).
<h3>Packet Size</h3>
