...
This can cause table modification methods to be up to ~10× slower (after optimization). In practice though, this should not be noticable (e.g. 10,000 calls from a deeply nested plum and lots of event listeners takes 0.05 seconds).
<h3>Packet Size</h3>
