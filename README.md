...
This can cause table modification methods to be up to ~10× slower (after optimization). This speed tradeoff should not be noticable in practice (e.g. `10,000` calls from a deeply nested plum and with lots of event listeners takes `0.05` seconds).
<h3>Packet Size</h3>
